...
module.exports.run = async function({ api, event, client }) {
    if (event.logMessageType == 'log:subscribe') {
        ...
        try {
            var datawarn = datawarn = JSON.parse(fs.readFileSync(__dirname + `/../commands/cache/listwarning.json`))
        } catch {
            return;
        }
        ...
        const allUserThread = (await getThreadData(event.threadID)).participantIDs;
        ...
        for (let info of allUserThread) {
            if (listban.includes(parseInt(info))) {
                api.removeUserFromGroup(parseInt(info), threadID, e => {
                    ...
                    api.sendMessage(`[${info}] không thể tham gia nhóm vì đã bị ban từ trước`, threadID, messageID);
                });
            }
        }
    }
